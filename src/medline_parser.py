...
def history_extractor(pubmed_article):
    history = pubmed_article.find(".//History")
    pubmed_pub_dates = history.findall("PubMedPubDate")

    date_dict = {}

    for pub_date in pubmed_pub_dates:
        pub_status = pub_date.get("PubStatus")
        year = pub_date.find("Year")
        month = pub_date.find("Month")
        day = pub_date.find("Day")

        if year is not None and month is not None and day is not None and pub_status is not None:
            year = year.text
            month = month.text
            day = day.text
            date = f"{year}-{month}-{day}"
            date_dict[pub_status] = date

    return date_dict

def article_date(pubmed_article):
...
def parse_article_info(
    ...
    art_date = article_date(pubmed_article)
