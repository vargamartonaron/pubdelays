...
result <- data |>
  dplyr::mutate(
    article_date_month = paste(
      lubridate::year(article_date),
      lubridate::month(article_date), "01", sep = "-"
    )
  ) |>
...
histogram_plot_month <- data |>
  ...
  ggplot2::geom_bar() +
  ggplot2::labs(
    title = "Density plot of articles per month",
...
histogram_plot_year <- data |>
  ...
  ggplot2::geom_bar() +
  ggplot2::labs(
    title = "Density plot of articles per year",
    x = "Year",
...
write.csv(
  acceptance_1perc_bottom,
  "acceptance_delay_bottom_1_percent.csv",
  row.names = FALSE
)
write.csv(
  acceptance_1perc_top,
  "acceptance_delay_top_1_percent.csv",
  row.names = FALSE
)
...
acceptance_1perc_year <- data |>
  ...
  summarise(
    bottom_1 = quantile(acceptance_delay, 0.01, na.rm = TRUE),
    top_1 = quantile(acceptance_delay, 0.99, na.rm = TRUE)
  )
print(acceptance_1perc_year)
write.csv(
  acceptance_1perc_year,
  "acceptance_1perc_year.csv",
  row.names = FALSE
)
...
write.csv(
  publication_1perc_bottom,
  "publication_delay_bottom_1_percent.csv",
  row.names = FALSE
)
write.csv(
  publication_1perc_top,
  "publication_delay_top_1_percent.csv",
  row.names = FALSE
)
...
publication_1perc_year <- data |>
  ...
  group_by(article_date_year) |>
  summarise(
    bottom_1 = quantile(publication_delay, 0.01, na.rm = TRUE),
    top_1 = quantile(publication_delay, 0.99, na.rm = TRUE)
  )
print(publication_1perc_year)
write.csv(
  publication_1perc_year,
  "publication_1perc_year.csv",
  row.names = FALSE
)
...
write.csv(journal_n_article, "journal_articles_n.csv", row.names = FALSE)
...
print(journal_n)
write.csv(journal_n, "journal_n.csv", row.names = FALSE)
...
result$article_date_month <- as.Date(
  result$article_date_month,
  format = "%Y-%m-%d"
)
ggplot(result, aes(x = article_date_month, y = acceptance_delay)) +
  ...
  labs(
    x = "Month",
    y = "Average Acceptance Delay",
    title = "Average Acceptance Delay by Month"
  ) +
  theme_minimal() +
...
ggplot(result, aes(x = article_date_month, y = acceptance_delay)) +
  geom_segment(aes(
    x = article_date_month,
    xend = article_date_month,
    y = 0,
    yend = acceptance_delay
  ), color = "red") +
...
articles_unfiltered <- jsonlite::fromJSON(
  "/users/usumusu/pubdelays/data/pubmed_medline_articles.json"
)
...
articles_unfiltered <- articles_unfiltered |>
  ...
  dplyr::mutate(
    acceptance_delay = as.numeric(difftime(accepted, received, units = "days"))
  ) |>
  dplyr::mutate(
...
min_date <- min(
  min(articles_unfiltered_compare$article_date_month),
  min(articles_filtered_compare$article_date_month)
)
max_date <- max(
  max(articles_unfiltered_compare$article_date_month),
  max(articles_filtered_compare$article_date_month)
)

ggplot() +
  geom_line(
    data = articles_filtered_compare,
    aes(x = article_date_month, y = acceptance_delay),
    color = "blue"
  ) +
  geom_line(
    data = articles_unfiltered_compare,
    aes(x = article_date_month, y = acceptance_delay),
    color = "red"
  ) +
...
articles_unfiltered$article_date_month <- as.Date(
  articles_unfiltered$article_date_month,
  format = "%Y-%m-%d"
)
articles_filtered$article_date_month <- as.Date(
  articles_filtered$article_date_month,
  format = "%Y-%m-%d"
)
...
ggplot() +
  geom_density(
    aes(x = acceptance_delay, color = "Full Dataset"),
    data = articles_unfiltered
  ) +
  geom_density(
    aes(x = acceptance_delay, color = "Subset Dataset"),
    data = articles_filtered
  ) +
  ...
  scale_color_manual(
    name = "Dataset",
    values = c("Full Dataset" = "blue", "Subset Dataset" = "red")
  )
...
ks_test_result <- ks.test(
  articles_unfiltered$acceptance_delay,
  articles_filtered$acceptance_delay
)
...
data |>
  ...
  ggplot(
    aes(
      x = month,
      y = mean_acceptance_delay,
      colour = factor(article_date_year)
    )
  ) +
...
result <- data |>
  ...
  mutate(
    ...
    group = case_when(
      article_number <= quantiles[1] ~ "Bottom 25%",
      article_number > quantiles[1] &
        article_number <= quantiles[2] ~ "Middle 50%",
      article_number > quantiles[2] ~ "Upper 25%"
    )
  ) |>
  select(-quantiles) |>
