...
| `transform-shard` | `transform-shard` | `transform_inputs.txt`, shard index, shard count, processed external metadata, and optional peer-review metadata | `articles-shard-<index>-of-<shards>.<format>` plus `.filters.csv` | `records` is `final_rows`; metadata contains filter counts, `shard_index`, `shards`, and selected input count | Skips complete shard output |
